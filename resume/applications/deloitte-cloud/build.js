const {
  ...
  LevelFormat, TabStopType, TabStopPosition, BorderStyle, PageBreak
...
function p(text, opts) {
  return new Paragraph({
    ...
    children: [new TextRun({ text, font: FONT, size: opts?.size || BODY_SIZE, bold: opts?.bold, italics: opts?.italic })]
  });
}

function blank(before) {
  return new Paragraph({ spacing: { before: before || 200, after: 0 }, children: [new TextRun("")] });
}
...
function jobHeader(company, location, title, dates) {
  return [
    ...
    new Paragraph({
      ...
      children: [new TextRun({ text: title, font: FONT, size: BODY_SIZE, italics: true })]
    })
  ];
}
...
const coverLetter = [
  new Paragraph({
    ...
    children: [new TextRun({ text: "Aditya Mehrotra", font: FONT, size: NAME_SIZE, bold: true })]
  }),
  new Paragraph({
    spacing: { before: 0, after: 0 },
    children: [new TextRun({ text: "Melbourne, Australia  |  +61 478 916 329  |  mehrotra.aditya16@outlook.com  |  linkedin.com/in/aditya-mehrotra-it  |  github.com/coded-by-aditya", font: FONT, size: SMALL_SIZE })]
  }),

  blank(300),
  p("4 July 2026"),
  blank(300),
  p("Re: Cloud Engineer – Req 41166, Deloitte Audit & Assurance", { bold: true }),
  blank(300),
  p("Dear Hiring Manager,"),
  blank(200),
  p("I'm applying for the Cloud Engineer role in the Audit & Assurance Data & Digital team. I was introduced to this opportunity through Viraj Soman, Senior Manager at Deloitte."),
  blank(200),
  p("Most of what the role describes is what I've been doing. The CI/CD and infrastructure work is the clearest overlap. On ShieldWalk, a women's safety app I built at Monash, I set up five GitHub Actions workflows from scratch covering OIDC authentication, per-developer ephemeral AWS environments via Terraform workspaces, automatic teardown on PR merge, and a saved-plan apply strategy to prevent plan-drift failures. The infrastructure side was also mine: a self-registering Terraform system that auto-discovered all 15 Lambda functions from config files and provisioned API Gateway routes, CloudWatch log groups, and S3 packaging with no manual steps per endpoint."),
  blank(200),
  ...
  blank(200),
  p("I would be glad to talk through the role."),
  blank(300),
  p("Sincerely,"),
  blank(100),
  p("Aditya Mehrotra", { bold: true }),
  p("+61 478 916 329"),
  p("mehrotra.aditya16@outlook.com"),
];
...
const resume = [
  ...
  ...educationEntry("Monash University", "Melbourne, Australia", "Master of Information Technology  |  WAM: 82%", "2026"),
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [...coverLetter, ...resume]
  }]
});

Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync(__dirname + "/application.docx", buffer);
  console.log("Done: deloitte-cloud/application.docx");
}).catch(console.error);
